    ...
    public static void main(String[] args) {
        ...
        for (int Articulo = 1; Articulo <= GananciaArticulos.length; Articulo++) {
            ...
            for (int CapacidadMaletaI = CapacidadMaletaMax; CapacidadMaletaI >= 1; CapacidadMaletaI--) {
                ...
                SolucionGanancias[CapacidadMaletaI] = Math.max(GananciaAnterior, GananciaArticuloI); // Ganancia anterior vs (Ganancia del articulo I + suma posible)

                if (GananciaArticuloI >= GananciaAnterior) {
                    SolucionArticulos[CapacidadMaletaI] = Articulos[Articulo - 1] + ","; //Nomnre Articulo en cuestion
                    SolucionArticulos[CapacidadMaletaI] += SolucionArticulos[CapacidadRestante];// le sumamos la solucion de articulo(s) anteriores posibles
                } else {
                    SolucionArticulos[CapacidadMaletaI] = SolucionArticulos[CapacidadMaletaI]; // Agregamos la solucion de articulo(s) anterior
                }
